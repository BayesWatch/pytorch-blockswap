...
def generate_archs():
    ...
    opts = list(
        string_to_conv.values()
    )  # string_to_conv lives in utils.py and is a dictionary of available convs
    for o in opts:
        params = []
    ...
    def get_param_tot(convs):
        ...
        return (
            conv_tot + 1978
        )  # 1978 is the number of the parameters in uncounted layers at the start/end of the net
...
for cand_id, candidate in enumerate(candidates):
    ...
    if cand_id % 10 == 0:
        print(
            ...
            "TotalTime {total_time.val:.3f} ({total_time.avg:.3f})".format(
                cand_id,
                args.samples,
...
best = pd.DataFrame({"index": [0], "convs": [best_geno], "fisher": [0.0]})
