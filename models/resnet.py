...
class ResNet(nn.Module):
    def __init__(
        ...
        self.block4.append(
            NetworkBlock(
                layers[3],
                nChannels[3] if i == 0 else nChannels[4],
                nChannels[4],
                2 if i == 0 else 1,
                dropRate,
                convs[l : l + layers[3]],
...
def resnet34(
    pretrained=False, conv=Conv, block=BasicBlock, masked=False, convs=[Conv, Conv]
):
...
def resnet50(pretrained=False, conv=Conv, block=BottleBlock, masked=False):
    ...
    """
    model = ResNet(
        conv, block, [3, 4, 6, 3], nChannels=[64, 256, 512, 1024, 2048], masked=masked
    )
    print(model)
    if pretrained:
